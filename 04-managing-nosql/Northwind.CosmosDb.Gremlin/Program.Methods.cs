...
partial class Program
{
    ...
    static async Task<(int, double)> ExecuteGremlinScript(string script)
    {
        ...
        catch (Exception ex)
        {
            ...
            WriteLine("Error stack trace: {0}", arg0: ex.StackTrace);
        }
    ...
    static async Task CreateCosmosGraphResources()
    {
        ...
        try
        {
            using (CosmosClient client = new(
              ...
              authKeyOrResourceToken: useLocal ? primaryKeyLocal : primaryKeyCloud))
            {
        ...
        try
        {
            using (NorthwindContext db = new())
            {
              ...
              g.addV("product")
                ...
                .property("unitsInStock", {p.UnitsInStock?.ToString() ?? "0"})
                .property("reorderLevel", {p.ReorderLevel?.ToString() ?? "0"})
                .property("unitsOnOrder", {p.UnitsOnOrder?.ToString() ?? "0"})
